...
export default function App() {
  const [dark, setDark] = useState(() => localStorage.getItem('physicsforge-chapter1-theme') === 'dark')
  const [active, setActive] = useState('home')
  const [menuOpen, setMenuOpen] = useState(false)

  useEffect(() => {
    document.documentElement.classList.toggle('dark', dark)
    localStorage.setItem('physicsforge-chapter1-theme', dark ? 'dark' : 'light')
  }, [dark])
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#f8fafc] text-ink transition-colors dark:bg-[#080e1b] dark:text-white">
      <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 md:px-6 md:pt-4">
        <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-[20px] border border-white/70 bg-white/80 px-4 py-3 shadow-[0_12px_40px_rgba(17,24,39,.08)] backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/80">
          <a href="#home" className="flex items-center gap-2.5" aria-label="PhysicsForge Chapter 1 home">
      ...
      <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[1fr_auto] md:items-end">
        <div>
          ...
          <p className="mt-4 max-w-xl text-sm leading-6 text-slate-500 dark:text-slate-400">
            Original Class 12 Electric Charges and Fields visual notes, simulations and practice. Built with React, Vite, Tailwind CSS and SVG using free/open-source tooling only.
          </p>
        </div>
